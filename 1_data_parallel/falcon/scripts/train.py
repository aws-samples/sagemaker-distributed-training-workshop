...
def get_arrow_files(folder_path):
    """
    Reads all the Apache Arrow (.parquet) files under the given folder path
...
def training_function(args):
    ...
    all_arrow_files = get_arrow_files(folder_path)
    
    # Print the list of Arrow file paths
    for file_path in all_arrow_files:
        print(file_path)

    #dataset = load_from_disk(args.dataset_path)

    dataset = load_dataset("arrow", data_files=all_arrow_files)
   ...
   # eval_dataset = dataset["validation"]
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for steps, batch in enumerate(tqdm(eval_dataloader,disable=not (args.rank==0))):
            ...
            print("executing steps:"+str(args.max_steps))
